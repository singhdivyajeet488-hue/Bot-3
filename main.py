...
# ============================================
async def get_minecraft_status(server_ip: str, port: int = 25565) -> Dict:
    try:
        from mcstatus import JavaServer
        
        # Try with timeout
        server = JavaServer(server_ip, port=port, timeout=5)
        ...
        try:
            ...
            return {
                ...
                'latency': ping or status.latency,
                'favicon': status.favicon if hasattr(status, 'favicon') else None
            }
        except Exception as e:
            # If status fails but ping works
            if ping:
                return {
                    'online': True,
                    'version': 'Unknown',
                    'players_online': '?',
                    'players_max': '?',
                    'motd': 'Server is online but status query failed',
                    'latency': ping,
                    'favicon': None
                }
            raise e
        ...
        if "Connect call failed" in error_msg:
            ...
            error_msg = "Connection timeout - server might be offline"
...
async def update_mc_status(bot: discord.Client, message_id: int, channel_id: int, server_ip: str, port: int = 25565):
    try:
        ...
        while True:
            try:
                ...
                status = await get_minecraft_status(server_ip, port)
                
                embed = create_mc_embed(server_ip, status, port)
...
def create_mc_embed(server_ip: str, status: Dict, port: int = 25565) -> discord.Embed:
    embed = discord.Embed(
    ...
    if status.get('online', False):
        ...
        if len(str(motd)) > 1024:
            motd = str(motd)[:1021] + "..."
        embed.add_field(name="Server MOTD", value=motd, inline=False)
        embed.add_field(
            name="📶 Latency", 
            value=f"{status.get('latency', 0):.1f}ms" if status.get('latency') else "N/A", 
            inline=False
        )
        ...
        embed.add_field(
            ...
            value="• Try adding port: `play.example.com:25565`\n"
                  "• Make sure server is online\n"
                  "• Some servers block status queries\n"
                  "• Try using Bedrock type for Bedrock servers",
            inline=False
        )
...
class DiscordBot(discord.Client):
    ...
    def register_commands(self):
        ...
        async def mcstatus(interaction: discord.Interaction, server_ip: str, port: Optional[int] = 25565):
            await self.handle_mcstatus(interaction, server_ip, port)
    ...
    # MC STATUS HANDLERS
    # ========================================
    async def handle_mcstatus(self, interaction: discord.Interaction, server_ip: str, port: int = 25565):
        await interaction.response.defer()
        
        # Show loading status
        loading_embed = discord.Embed(
            title="⏳ Checking Server Status...",
            description=f"Pinging {server_ip}:{port}",
        ...
        if status.get('online', False):
            task = asyncio.create_task(update_mc_status(self, message.id, interaction.channel_id, server_ip, port))
